...
const effectsList = [
	...
	[
		" (WASM) Frequency Spectrum: IIR Biquad Frequency Filter",
		"Filters out, attenuates or boosts audio frequencies at a specific bound.",
		'Form: <select id="wasm_biquadfilteri5"><option value="0">(0) ii trans (optimized for performance and memory)</option><option value="1">(1) ii (slightly performant, but numerically imprecise)</option><option value="2">(2) i trans (slightly performant and barely imprecise)</option><option value="3">(3) i (optimized for numerical stability and precision)</option></select><br>Frequency Cutoff: <input id="wasm_biquadfilteri0" type="number" min="0" max="24000" step="1" value="1000"> Hz<br>Quality (1 = smooth cutoff, 0.5 = dull cutoff, 0.1 = sharp cutoff): <input id="wasm_biquadfilteri1" type="number" min="0" max="1" step="0.01" value="1"><br>Type of Filter: <select id="wasm_biquadfilteri2" oninput="let sm=document.getElementById(\'gn\');if (this.value === \'3\' || this.value === \'5\' || this.value === \'6\') sm.style.display = \'\'; else sm.style.display = \'none\'; sm=document.getElementById(\'pr\'); if (this.value === \'4\') sm.style.display = \'\'; else sm.style.display = \'none\'; sm=document.getElementById(\'importantnote\'); if (this.value === \'7\') {sm.style.display = \'\';document.getElementById(\'wasm_biquadfilteri1\').value = Math.SQRT1_2;} else sm.style.display = \'none\';"><option value="0">(0) lowpass (attenuate higher frequencies)</option><option value="1">(1) highpass (attenuate lower frequencies)</option><option value="2">(2) bandpass (attenuates surrounding frequencies)</option><option value="3">(3) peak (amplifies or attenuates a frequency)</option><option value="4">(4) notch (attenuates a frequency)</option><option value="5">(5) lowshelf (amplifies or attenuates lower frequencies)</option><option value="6">(6) highshelf (amplifies or attenuates higher frequencies)</option><option value="7">(7) allpass (changes the phase)</option><option value="8">(8) sallen-key lowpass (attenuate higher frequencies via simulated simplicity)</option><option value="9">(9) sallen-key highpass (attenuate lower frequencies via simulated simplicity)</option></select><div id="gn" style="display: none">Gain: <input id="wasm_biquadfilteri3" type="number" min="-1000" max="1000" step="0.02" value="0"> Db<br></div><div id="pr" style="display: none">Pole Radius (1 = exceedingly narrow notch, 0.7 = wide notch): <input id="wasm_biquadfilteri4" type="number" min="0" max="1" step="0.01" value="0.7"><br></div><div id="importantnote" style="display:none"><hr><a>IMPORTANT: The allpass filter can be used to delay the timing of audio frequencies. The set quality is for a natural-sounding, smooth phase shift.</a></div><hr>Advantages:<br><ul><li>Stability and Performance: Performing only around 4 add/subtract and 5 multiplication operations per sample, the error accumulation is often tiny, and biquad filters are not heavy on the CPU. It\'s less likely for Biquad Filters to produce noise or static.</li><li>Flexibility: They come in different Direct Forms, each one having their advantages and disadvantages. Engineers can easily pick one that\'s suited for their environment, and requirements. If they want numerical stability regardless of memory, they can pick Direct Form I or transposed Direct Form I. If they care about memory, performance and stability, they can pick transposed Direct Form II.</li><li>Simplicity: If someone wants to add their own type of frequency filter, they can simply recompute the coefficients. They may need to understand programming, but the knowledge doesn\'t have to be deep.</li></ul><br>Disadvantages:<br><ul><li>Phase Distortion: Biquad Filters are Infinite Impulse Response (IIR) filters, which use feedback loops. They operate at the time domain, and don\'t keep track of the linearity of the phase. Some frequencies are delayed, while others are not.</li><li>Sensitive To Extrema-adjacent Parameters: Very high Q, or very low cutoffs may produce ill-conditioned coefficients, which can result in distorted audio.</li><li>Quantization: Since the internal feedback values or coefficients may have large magnitudes, depending on the floating point format used (WASM specifies IEEE 754 Floating Point precision), they can be prone to subtle quantization artifacts. This is especially when the audio is very quiet, or at an imperceptible volume.</li><li>Subtle Transitions Required: Changing coefficients to very different values can result in severe audio artifacts, or destabilization across multiple samples. Parameter-space interpolation reduces this problem significantly, but requires careful precision to pull off.</li><li>Deep Understanding: If someone wants to add their own type of frequency filter, they also need a deep understanding of Biquad filter formulas. Studying them is psychologically demanding, and so is designing the custom formulas for the coefficients.</li></ul><br>Comparison:<br><ul><li>Biquad Frequency filters can preserve resonance, but they flatten punchiness. They are good for filtering out frequencies and repairing audio, but not for boosting frequencies and keeping punchiness at the same time.</li><li>4-tap comb filters can preserve resonance and punchiness, and generate rich harmonics.</li></ul>',
		6,
		"wasm_biquadfilteri",
		[Number, Number, Number, Number, function(x){return 1 - Number(x)}, Number]
	],
	[
		" (WASM) Frequency Spectrum: IIR Comb Feedback Filter",
		"Boosts low or high frequencies using an IIR comb filter. The higher the multipler, the higher the boost. You may need to normalize the audio using the Normalize Audio effect.",
		'Feedback Multiplier: <input id="wasm_combfeedbackfilter0" type="number" min="-0.99" max="0.99" step="0.001" value="0.95"><br>Number of Taps: <input id="wasm_combfeedbackfilter2" type="number" min="1" max="128" step="1" value="4"><br>Type of Filter: <select id="wasm_combfeedbackfilter1"><option value="0">(0) lowpass (positive gain only)</option><option value="1">(1) highpass (positive gain only)</option></select><br>Advantages:<br><ul><li>Stability and Performance: A feedback comb filter usually performs at least 1 add/subtract operation. The error accumulation is negligible.</li><li>Simplicity: Comb filters are highly accessible. It\'s easy for someone to implement custom formulas without requiring a deep understanding.</li><li>Punchiness: Comb filters can preserve both the resonance, and punchiness of the original audio. They can also generate rich harmonics that are desired in experimental audio processing.</li></ul><br>Disadvantages:<br><ul><li>Normalization Requirements: The comb filter\'s feedback loop is added into the input audio signals, which can amplify, attenuate or cancel out the audio\'s frequencies.</li><li>Ringing Artifacts: Comb filters might generate metallic ringing, or unnatural high-pitched tones. This artifact is very perceptible in the highpass filter.</li><li>Phase Distortion: This implementation has IIR filter characteristics. IIR filters don\'t keep track of the linearity of the phase. Some frequencies are delayed, while others are not.</li><li>High Amounts of Taps: Comb feedback filters are designed to feed its output signals back into the input signals. Because of this, using a very high amount of taps can generate a perceptible echoing effect.</li></ul><br>Comparison:<br><ul><li>Comb filters can preserve resonance and punchiness, while generating rich harmonics that make percussion sound "punchy". However, depending on the implementation, this can amplify or attenuate the audio.</li><li>Biquad filters can preserve resonance, but don\'t preserve the punchiness, which can make the audio feel a bit flat. They do feed its output signals back into the input, but their coefficients are carefully (and precisely) calculated, which is why the audio\'s signals aren\'t noticeably stronger or weaker.</li></ul>',
		3,
	...
	[
		...
		'Noise Type: <select id="noise0"><option value="wn">white noise</option><option value="gr">gray noise</option><option value="black">black noise (audible)</option><option value="orn">orange noise</option><option value="yello">yellow noise</option><option value="green">green noise</option><option value="bln">blue noise</option><option value="vn">violet noise</option><option value="pn">pink noise (Voss-McCartney)</option><option value="pnacc">pink noise (accumulator-based)</option><option value="bn">brown noise</option></select><br>Volume: <input id="noise1" type="number" min="0" max="100" step="1" value="50" placeholder="Hover for info" style="width: 100px" title="How loud the static is.">%<br>Is "Algorithmistic": <input id="noise2" type="checkbox" checked title="Determines whether their noise\'s implementations adhere to faithful algorithms, or default to generic or approximate programs.">',
		3,
		"noise",
	...
	[
		...
		"Analyzes the audio data in chunks using a Fast Fourier Transform algorithm (indutny FFT.js), then pitch shifts the audio by accumulating the phase, and linearly distributing the magnitude. This effect has a noticeably less robotic quality, but may have some metallic \"chorus\"-like artifacts, which are reduced using the \"polished\" setting.<br>If you want to apply pitch shifting on this audio track, switch to the Phase Vocoder Mono setting.<br><b>This variant may be ~3 times slower than the mono variant.</b>",
	...
	[
		...
		'FFT Size: <input id="fftsaturationsmear0" type="number" min="32" step="32" value="2048" style="width: 100px"><br>Interpolation: <input id="fftsaturationsmear1" type="number" min="1" step="1" value="50" style="width: 100px">%<br>Interpolation Direction: <select id="fftsaturationsmear2"><option value="btou">lower -> upper</option><option value="utop">upper -> lower</option></select>',
		3,
		"fftsaturationsmear",
