...
const effectsList = [
	...
	[
		" (WASM) Frequency Spectrum: IIR Comb Filter (BETA)",
		...
		'Feedback Multiplier: <input id="wasm_combfeedbackfilter0" type="number" min="-0.99" max="0.99" step="0.001" value="0.95"><br>Number of Taps: <input id="wasm_combfeedbackfilter2" type="number" min="1" max="128" step="1" value="4"><br>Type of Filter: <select id="wasm_combfeedbackfilter1"><option value="0">(0) lowpass (positive gain only)</option><option value="1">(1) highpass (positive gain only)</option></select><br>Advantages:<br><ul><li>Stability and Performance: This depends on the formulas of the filters. This implementation\'s filters perform 1 add/subtract operation, and 1 multiplication operation. The error accumulation is negligible.</li><li>Simplicity: Comb filters are highly accessible. It\'s easy for someone to implement custom formulas without requiring a deep understanding of comb filters.</li><li>Punchiness: Comb filters can preserve both the resonance, and punchiness of the original audio. They can also generate rich harmonics.</li></ul><br>Disadvantages:<br><ul><li>Normalization Requirements: Again, this depends on the formulas of the filters. In this implementation, the 4-tap feedback loop is added into the audio track, which may boost the audio\'s volume.</li><li>Ringing Artifacts: Comb filters might generate metallic ringing, or unnatural high-pitched tones. This is significant in the highpass filter.</li><li>Phase Distortion: This implementation has IIR filter characteristics. IIR filters don\'t keep track of the linearity of the phase. Some frequencies are delayed, while others are not.</li></ul><br>Comparison:<br><ul><li>Comb filters can preserve resonance and punchiness, and generate rich harmonics that make percussion sound "punchy". However, depending on the implementation, this can also amplify or attenuate the audio.</li></ul>',
		3,
